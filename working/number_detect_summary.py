...
def format_range(start_page, start_line, end_page, end_line):
    if start_page == end_page:
        if start_line == end_line:
            return f"{start_page}:{start_line}"
        return f"{start_page}:{start_line}-{end_line}"
    return f"{start_page}:{start_line}-{end_page}:{end_line}"


def print_color_ranges(color_ranges, output_file):
    with open(output_file, "w") as f:
        for color, ranges in color_ranges.items():
            f.write(f"[{color}]\n")
            for r in ranges:
                f.write(f"  {format_range(*r)}\n")
            f.write("\n")  # Add a space after each color section


def print_page_9_line_numbers(page_9_lines, output_file):
    with open(output_file, "a") as f:
        f.write("\nLine numbers on Page 9:\n")
...
# Example usage
pdf_path = 'demo2.pdf'
output_file = 'highlighted_sections.txt'
...
print_color_ranges(consolidated_ranges, output_file)
print_page_9_line_numbers(page_9_lines, output_file)
