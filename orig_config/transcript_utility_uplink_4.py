...
# Check if Docker secret exists, otherwise fall back to .env file
if not os.path.exists(CONFIG_PATH):
    CONFIG_PATH = os.getenv("CONFIG")

# Raise an error if the file is not found
if not CONFIG_PATH or not os.path.exists(CONFIG_PATH):
    raise FileNotFoundError("Service account file not found. Check if it's available in /run/secrets or .env file.")

# Load the JSON content from the file
with open(CONFIG_PATH, 'r') as config_file:
    CONFIG = json.load(config_file)

# Access the Anvil Uplink Key
anvil_uplink_key = CONFIG.get('ANVIL_UPLINK_KEY')

# Raise an error if the key is not found in the JSON
if not anvil_uplink_key:
    raise KeyError("ANVIL_UPLINK_KEY not found in the config file.")
